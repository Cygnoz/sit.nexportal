...
const Select = forwardRef<HTMLSelectElement, SelectProps>(
  ({ label, options, error, placeholder, required, readOnly, ...props }, ref) => {
    return (
      <div className="relative w-full">
        ...
        >
          <select
            ...
            className={`block appearance-none w-full h-9 text-[#818894] bg-white border 
                        text-sm pl-2 pr-8 rounded-md leading-tight 
                        focus:outline-none focus:bg-white 
                        ${error ? "border-red-500" : "border-gray-300"}`}
            disabled={readOnly} // Set disabled when readOnly is true
            {...props}
          >
